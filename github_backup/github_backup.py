...
def main():
    ...
    if (
        ...
        or args.include_followers
        or args.include_following
        or args.include_keys
    ):
        args.account = True
    ...
    if args.password == False:
        ...
        if os.path.isfile(CONFFILE):
            ...
            password = getpass.getpass(
                "Enter password for {}: ".format(config["login_or_token"])
            )
            if password:
    ...
    IS_AUTHORIZED = isinstance(
        account,
        (github.AuthenticatedUser.AuthenticatedUser, github.Organization.Organization),
    )
...
def init_parser():
    ...
    parser.add_argument(
        "login_or_token", help="A Github username or token for authenticating"
    )
    parser.add_argument(
        "backupdir", help="The folder where you want your backups to go"
    )
    parser.add_argument(
        "-v",
        "--visibility",
        help="Filter repos by their visibility",
    ...
    parser.add_argument(
        "-a",
        "--affiliation",
        help="Filter repos by their affiliation",
        action="append",
        type=str,
        default=["owner"],
    ...
    parser.add_argument(
        "-m", "--mirror", help="Create a bare mirror", action="store_true"
    )
    parser.add_argument("-f", "--skip-forks", help="Skip forks", action="store_true")
    parser.add_argument(
        "--skip-repos", help="Skip backing up repositories", action="store_true"
    )
    parser.add_argument(
        "-g",
        "--git",
    ...
    parser.add_argument(
        "-t",
        "--type",
        help="Select the protocol for cloning",
        choices=["git", "http", "ssh"],
        default="ssh",
    )
    parser.add_argument(
        "-s", "--suffix", help="Add suffix to repository directory names", default=""
    )
    parser.add_argument("-u", "--username", help="Backup USER account", metavar="USER")
    parser.add_argument(
        "-p",
        "--password",
        help="Authenticate with Github API (give no argument to check ~/.github-backup.conf or prompt for a password)",
        nargs="?",
        default=False,
    )
    parser.add_argument(
        "-P", "--prefix", help="Add prefix to repository directory names", default=""
    )
    parser.add_argument(
        "-o", "--organization", help="Backup Organizational repositories", metavar="ORG"
    )
    parser.add_argument(
        "-A", "--account", help="Backup account data", action="store_true"
    )
    parser.add_argument(
        "--all",
        action="store_true",
...
def fetch_url(url, outfile):
    ...
    with open(outfile, "w") as f:
        ...
        LOGGER.debug(
            "GET %s %r ==> %d %r", url, headers, resp.status_code, resp.headers
        )
...
def process_account(gh, account, args):
    ...
    if args.include_watched:
        ...
        fetch_url(
            get_account_subscriptions_url(account), os.path.join(dir, "watched.json")
        )

    if args.include_followers:
        LOGGER.info("    Getting followers repository list")
        fetch_url(
            get_account_followers_url(account), os.path.join(dir, "followers.json")
        )

    if args.include_following:
        LOGGER.info("    Getting following repository list")
        fetch_url(
            get_account_following_url(account), os.path.join(dir, "following.json")
        )
...
class RepositoryBackup(object):
    def __init__(self, repo, args):
        ...
        if self.is_gist:
            ...
            if re.match("^ghp_.+$", args.login_or_token):
                url = re.sub(
                    "^https://",
                    f"https://{args.login_or_token}:x-auth-basic@",
                    repo.clone_url,
                )
            elif args.password:
                if args.username:
                    url = re.sub(
                        "^https://",
                        f"https://{args.username}:{args.password}@",
                        repo.clone_url,
                    )
                else:
                    url = re.sub(
                        "^https://",
                        f"https://{args.login_or_token}:{args.password}@",
                        repo.clone_url,
                    )
            else:
                url = repo.clone_url
    ...
    def backup(self):
        ...
        if self.wiki_url:
            config = os.path.join(
                self.wiki_dir, "config" if self.args.mirror else ".git/config"
            )
    ...
    def update_repo(self, dir):
        ...
        if repo.description:
            git(
                "config",
                ["--local", "gitweb.description", repo.description.encode("utf-8")],
                gdir=dir,
            )

        if repo.owner.name and repo.owner.email:
            owner = "%s <%s>" % (
                repo.owner.name.encode("utf-8"),
        ...
        if self.is_gist:
            ...
            git(
                "config",
                ["--local", "cgit.clone-url", str(repo.git_pull_url)],
                gdir=dir,
            )
        else:
            ...
            git(
                "config",
                ["--local", "cgit.defbranch", str(repo.default_branch)],
                gdir=dir,
            )
        ...
        for issue in issues:
            ...
            if args.include_issue_comments and issue.comments:
                ...
                for comment in get_issue_comments(issue):
                    issue_data.setdefault("comment_data", []).append(
                        get_comment_raw_data(comment)
                    )
            if args.include_issue_events:
                for event in get_issue_events(issue):
                    issue_data.setdefault("event_data", []).append(
                        get_event_raw_data(event)
                    )

            issue_file = os.path.join(
                dir, "issues", "{0}:{1}.json".format(project, issue.number)
            )
        ...
        for issue in issues:
            ...
            if args.include_pull_commits and issue.commits:
                for commit in get_issue_commits(issue):
                    issue_data.setdefault("commit_data", []).append(
                        get_commit_raw_data(commit)
                    )

            issue_file = os.path.join(
                dir, "pull-requests", "{0}:{1}.json".format(project, issue.number)
            )
            if not os.access(os.path.dirname(issue_file), os.F_OK):
                mkdir_p(os.path.dirname(issue_file))
            with codecs.open(issue_file, "w", encoding="utf-8") as f:
